...
MANIFEST = {
    ...
    "version": "1.1.0",
    "name": "GDrive",
    "description": "This plugin fetches content from google drive.",
...
def addon_stream(stream_type, stream_id):
    ...
    try:
        ...
        resp = Response(
            response=get_streams(stream_type, stream_id, range_header),
...
def get_streams(stream_type, stream_id, range_header=None):
    ...
    # Set up partial content headers if range is provided
    if range_header:
        byte_range = range_header.replace("bytes=", "").split("-")
        start = int(byte_range[0])
        end = int(byte_range[1]) if byte_range[1] else os.path.getsize(file_path) - 1
        length = end - start + 1
        ...
        # Code for partial content reading from gdrive here, if applicable.
